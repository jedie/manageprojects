...
class PublisherGit:
    def __init__(
        ...
        self.change_msg = None  # Set later
    ...
    def finalize(self):
        changes = '\n'.join(self.change_msg)
        version_str = f'v{self.version}'
        self.git.tag(
            version_str, message=f'publish version {version_str} with these changes:\n{changes}', verbose=True
        )
...
def build(package_path) -> None:
    ...
    with tempfile.NamedTemporaryFile(
        mode='w', suffix='.txt', prefix=package_path.name, encoding='utf-8', delete=False
    ) as temp_file:
        temp_file.write(output)
    print(f'Output written to: {temp_file.name}')
